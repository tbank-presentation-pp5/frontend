...
import { GenerateOutlineRequest, Presentation } from "../types";
import { PresentationOutline } from "@/features/presentation-outline/store/usePresentationOutlineStore";
...
export const generatePresentationOutline = async (
	...
	try {
		const response = await fetch(
			`${process.env.NEXT_PUBLIC_API_URL}/api/v1/presentation-plans/generate`,
			{
				method: "POST",
				headers: {
					"Content-Type": "application/json",
...
export const getPresentationById = async (
	...
	const response = await fetch(
		`${process.env.NEXT_PUBLIC_API_URL}/api/v1/presentations/${id}`,
	);
...
export const getPresentationOutline = async (
	id: number,
): Promise<PresentationOutline> => {
	try {
		const response = await fetch(
			`${process.env.NEXT_PUBLIC_API_URL}/api/v1/presentation-plans/${id}`,
			{
				method: "GET",
				headers: {
					"Content-Type": "application/json",
...
export const updatePresentationPlan = async (
	...
	try {
		...
		if (!response.ok) {
			throw new Error(
				`Failed to update presentation plan: ${response.statusText}`,
			);
		}
