...
export function Card({ variant }: CardProps) {
  ...
  return (
    <motion.div initial={{ scale: 0.5, opacity: 0.5 }} animate={{ scale: 1, opacity: 1 }} transition={{ ease: 'easeInOut' }}>
      <div
      ...
      >
        ...
        <div className="px-5 py-10 flex flex-col items-center">
          <h2 className="font-medium font-tinkoff text-2xl mb-3">
            {content.h2}
          </h2>
          ...
          <Link href={content.link}>
            <Button
              variant="white"
              onClick={() => usePromptStore.getState().setPage("ai")}
            >
              {content.button_text}
            </Button>
          </Link>
